...
def generate_control_file(version):
    ...
    content = """Package: bertdesk
Section: net
Priority: optional
Version: %s
Architecture: %s
Maintainer: bertdesk <info@bertdesk.com>
Homepage: https://bertdesk.com
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/share/bertdesk')
    system2('mkdir -p tmpdeb/etc/bertdesk/')
    system2('mkdir -p tmpdeb/etc/pam.d/')
    system2('mkdir -p tmpdeb/usr/share/bertdesk/files/systemd/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
    system2('mkdir -p tmpdeb/usr/share/applications/')
    system2('mkdir -p tmpdeb/usr/share/polkit-1/actions')
    system2('rm tmpdeb/usr/bin/bertdesk || true')
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/usr/share/bertdesk/')
    system2(
        'cp ../res/bertdesk.service tmpdeb/usr/share/bertdesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/bertdesk.png')
    system2(
        'cp ../res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/bertdesk.svg')
    system2(
        'cp ../res/bertdesk.desktop tmpdeb/usr/share/applications/bertdesk.desktop')
    system2(
        'cp ../res/bertdesk-link.desktop tmpdeb/usr/share/applications/bertdesk-link.desktop')
    system2(
        'cp ../res/startwm.sh tmpdeb/etc/bertdesk/')
    system2(
        'cp ../res/xorg.conf tmpdeb/etc/bertdesk/')
    system2(
        'cp ../res/pam.d/bertdesk.debian tmpdeb/etc/pam.d/bertdesk')
    system2(
        "echo \"#!/bin/sh\" >> tmpdeb/usr/share/bertdesk/files/polkit && chmod a+x tmpdeb/usr/share/bertdesk/files/polkit")

    system2('mkdir -p tmpdeb/DEBIAN')
    generate_control_file(version)
    system2('cp -a ../res/DEBIAN/* tmpdeb/DEBIAN/')
    md5_file_folder("tmpdeb/")
    system2('dpkg-deb -b tmpdeb bertdesk.deb;')

    system2('/bin/rm -rf tmpdeb/')
    system2('/bin/rm -rf ../res/DEBIAN/control')
    os.rename('bertdesk.deb', '../bertdesk-%s.deb' % version)
    os.chdir("..")
...
def build_deb_from_folder(version, binary_folder):
    ...
    system2('rm tmpdeb/usr/bin/bertdesk || true')
    system2(
        f'cp -r ../{binary_folder}/* tmpdeb/usr/share/bertdesk/')
    system2(
        'cp ../res/bertdesk.service tmpdeb/usr/share/bertdesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/bertdesk.png')
...
def build_flutter_dmg(version, features):
    ...
    system2('cp -rf ../target/release/service ./build/macos/Build/Products/Release/BertDesk.app/Contents/MacOS/')
    # re-sign ad-hoc: copying files in breaks the bundle signature ("app is damaged")
    system2('codesign --force --deep --sign - ./build/macos/Build/Products/Release/BertDesk.app')
    '''
    system2(
